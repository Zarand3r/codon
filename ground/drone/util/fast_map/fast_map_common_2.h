...
// Fixed-size, linear-probing hash table template, optimized for U32->U32, SIMD, and cache alignment
...
public:
	...
	FASTMAP_FORCE_INLINE bool Insert(U32 key, U32 value) {
		...
		for (U64 probe = 0; probe < kInternalCap; probe += kElemsPerBucket) {
			...
			int mask = _mm_movemask_ps(_mm_castsi128_ps(_mm_cmpeq_epi32(*bucket, keyvec)));
			if (mask) {
				int offset = __builtin_ctz(mask);
				m_values[idx + offset] = value;
				return true;
			}
			mask = _mm_movemask_ps(_mm_castsi128_ps(_mm_cmpeq_epi32(*bucket, emptyvec)));
			if (mask) {
				int offset = __builtin_ctz(mask);
				if (m_size >= kCapacity) return false;
	...
	FASTMAP_FORCE_INLINE U32* Find(U32 key) {
		...
		for (U64 probe = 0; probe < kInternalCap; probe += kElemsPerBucket) {
			...
			__m128i emptyvec = _mm_set1_epi32(kInvalidU32);
			int emptymask = _mm_movemask_ps(_mm_castsi128_ps(_mm_cmpeq_epi32(*bucket, emptyvec)));
			if (emptymask) return nullptr;
			idx = (idx + kElemsPerBucket) & kInternalCapMask;
		}
		return nullptr;
	}
