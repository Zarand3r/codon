...
| `slate_accessor.h` (imported, **stub**) | D2 path fix (`core/slate_info.h`→`slate_info.h`); real content (`SlateAccessor<T>`, `slate_validator_t`, validators) still to build | — | next blocker for `SlateLayout`: the validator/accessor subsystem |
| validator/accessor subsystem (`slate_validator_t`, `SlateAccessor<T>`, `enum_validator`, `SlateTypedValidator`), `SlateBuilderStore`, `SlateMemory`, `EnumRegistry`, `Slate`, `SlateCombiner` | — | — | pending (next P1 increments) |
