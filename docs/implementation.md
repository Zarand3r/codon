...
| `SlateElement`, `SlatePathMap`, `SlateBuilderStore`, `SlateMemory`, `EnumRegistry`, `Slate`, `SlateCombiner` | — | — | pending (next P1 increments) |

## Verification status (current)

- **g++** `-Wall -Wextra -Werror`: **15 unit tests + 2 consumer-compiles** green (`scripts/run_l0_tests.sh`).
