// TDD (P0): fswtime contract — nano_t, sentinels, monotonic clock, sleep.
...
int main()
{
    ...
    fswsleep(0);                 // non-positive returns immediately
    fswsleep(-5);                // negative returns immediately
    const nano_t a = get_rel_time();
    fswsleep(2 * 1000 * 1000);   // 2 ms
    const nano_t b = get_rel_time();
    assert(b - a >= 1 * 1000 * 1000); // at least ~1 ms elapsed

    std::printf("fswtime_test: PASS\n");
    return 0;
}
